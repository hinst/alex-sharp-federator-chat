...
class App {
    ...
    ignoredWords: string[] = ['https', 'like', 'fedir', 'holder', 'http', 'tsapana'];
    ...
    run() {
        ...
        const topEntries = sortedDictionary
            .filter(entry => entry[0].length > 1)
            .filter(entry => !this.ignoredWords.includes(entry[0]))
            .slice(10, 110);
        console.log(topEntries);
        const topWords = topEntries.map(entry => entry[0]);
    ...
    private loadTelegramMessages() {
        ...
        for (const message of messages) {
            if (message.from_id == this.myTelegramId && typeof message.text === 'string')
    ...
    private loadSkypeMessages() {
        const conversationPartnerId = '8:truefedex';
        ...
        const conversation = allMessages.conversations.find(
            (conversation: any) => conversation.id == conversationPartnerId
        );
        const messages: SkypeMessage[] = conversation.MessageList;
        console.log('Skype messages: ' + messages.length);
        for (const message of messages)
            if (message.content && message.from == this.mySkypeId) {
                if (message.messagetype == 'Text')
                    this.addMessage(message.content);
                else if (message.messagetype == 'RichText') {
                    const dom = new JSDOM('<!DOCTYPE html><p>' + message.content + '</p>');
